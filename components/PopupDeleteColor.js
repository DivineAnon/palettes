...
import { deleteDashboardColor, removeDetailDashboardCollectionColors, removeDetailDashboardProjectColorsData, removeFromFavorite } from "../slices/dashboardSlice";
import { selectIdDeleteColor, setDataDeleteColor } from "../slices/popupSlice";
import ContainerPopup from "./ContainerPopup";
import Spinner from "./Spinner";

export default function PopupDeleteColor(){
    const dispatch = useDispatch();
    const deleteColorId = useSelector(selectIdDeleteColor);
    ...
    const handleDelete = async () => {
        setLoading(true);
        const color = await axios.delete(`${process.env.NEXT_PUBLIC_API}/api/saves-colors/delete/${deleteColorId}`,{
            headers: {
        ...
        if (Router.pathname==='/user/colors') {
            ...
            dispatch(removeFromFavorite({ id, type: 'colors' }));
        }
        setLoading(false);
    ...
    <ContainerPopup remove={removeBox}>
        <div ref={elementRef} className="animate-translateY sm:animate-fadeIn bg-white w-full sm:w-[380px] sm:relative absolute bottom-0 rounded-tl-xl rounded-tr-xl sm:rounded-xl flex flex-col divide-y">
            <div className="p-3 relative">
                ...
                <h1 className="font-semibold text-center">Delete Color</h1>
            </div>
            <div className="min-h-[100px] flex items-center justify-center p-6">
                <p className="text-[15px] font-medium">Do you really want to delete this color?</p>
            </div>
