...
import { handleSaveGradient, setDataAddToFav, setDataDelFromFav, setDataFullscreenPalette, setDataMenuMore, setDataShowCSSGradient, setIdDeleteGradient } from "../slices/popupSlice";
import { selectUser } from "../slices/userSlice";
import { selectCopyPaletteIndex, setCopyPaletteIndex } from "../slices/globalSlice";

export default function PaletteGradientSaves({ data, pc, fav }){
    ...
    const handleMenuMore = (menu) => {
        if (menu==='openGradient') {
            window.open(`/user/gradients/${data.id}`)
        }
        if (menu==='duplicate') {
        ...
        if (menu==='addProjects') {
            dispatch(handleSaveGradient(user,data.gradient,'edit',data,null,'projects'));
        }
        ...
        if (menu==='saveToFavorite') {
            dispatch(setDataAddToFav({ id: data.id, type: 'gradients' }));
        }
        if (menu==='removeToFavorite') {
            dispatch(setDataDelFromFav({ id: data.id, type: 'gradients' }));
        }
    }
    const menuMore = ()=> (
        <Fragment>
            <section className="mb-2">
                <div onClick={()=>handleMenuMore('openGradient')} className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded-md cursor-pointer">
                    ...
                    <span className="md:text-sm font-medium">Open gradient</span>
                </div>
            ...
            <section className={`pt-2 ${!pc && 'mb-2'}`}>
                <div onClick={()=>handleMenuMore('duplicate')} className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded-md cursor-pointer">
                    ...
                    <span className="md:text-sm font-medium">Duplicate gradient</span>
                </div>
