...
import { removeFromFavorite } from "../slices/dashboardSlice";
import { selectDataDeleteFromFav, setDataDelFromFav } from "../slices/popupSlice";
import ContainerPopup from "./ContainerPopup";
import Spinner from "./Spinner";

export default function PopupDeleteFromFav(){
    const dispatch = useDispatch();
    const elementRef = useRef(null);
    const data = useSelector(selectDataDeleteFromFav);
    ...
    const removeBox = (time) => {
        ...
        const close = setTimeout(()=>{
            dispatch(setDataDelFromFav(null));
        },time);
        return ()=> clearTimeout(close);
    }
    const handleRemove = async () => {
        setLoading(true);
        const del = await axios.put(`${process.env.NEXT_PUBLIC_API}/api/favorites/delete`,{
            ...data
        },{
            headers: {
        ...
        dispatch(setDataDelFromFav(null));
        handlePushNotif({ text: `${data.type.charAt(0).toUpperCase() + data.type.slice(1,-1)} deleted from favorite!`, className: 'bg-black', icon: 'checklist' });
    }
    return (
        ...
        <div ref={elementRef} className="animate-translateY sm:animate-fadeIn bg-white w-full sm:w-[380px] sm:relative absolute bottom-0 rounded-tl-xl rounded-tr-xl sm:rounded-xl flex flex-col divide-y">
            <div className="p-3 relative">
                ...
                <h1 className="font-semibold text-center">Delete from favorite</h1>
            </div>
            <div className="min-h-[100px] flex items-center justify-center p-6">
                <p className="text-[15px] font-medium">Delete this {data.type.slice(0,-1)} from favorite?</p>
            </div>
            <div className="flex p-4 gap-3">
                ...
                <button onClick={handleRemove} className="h-[46px] text-center font-bold bg-red-500 hover:bg-red-600 transition text-white w-full block rounded-xl">{loading ? <Spinner/> : 'Delete'}</button>
            </div>
        </div>
    </ContainerPopup>
    )
