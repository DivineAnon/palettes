...
import { setDataAddToFav, setDataDelFromFav, setDataMenuMore, setDataPopupCollection, setIdDeleteCollection } from "../slices/popupSlice";

export default function CollectionSaves({ data, fav }){
    ...
    const handleMenuMore = (menu) => {
        ...
        if (menu==='saveToFavorite') {
            dispatch(setDataAddToFav({ id: data.id, type: 'collections' }));
        }
        if (menu==='removeToFavorite') {
            dispatch(setDataDelFromFav({ id: data.id, type: 'collections' }));
        }
    }
    const menuMore = ()=> (
        <Fragment>
