...
import { DashboardLoading, DashboardLoadingSearch, DashboardSearch, DashboardTemplate, ProjectSaves } from "../../../components";
...
export default function Projects(){
    ...
    const fetchProjectsSearch = async (query) => {
        ...
        const projects = await axios.get(`${process.env.NEXT_PUBLIC_API}/api/projects/feed?${getQuery(query,1)
        }`,{
            headers: {
        ...
        dispatch(setDashboardProjects(projects.data));
    }
    const filtersMenu = () => (
    ...
    const fetchProjects = () => {
        dispatch(fetchMoreProjectsDashboard(getQuery(query,parseInt(projectsPage)+1)));
    }
    return (
        <DashboardTemplate>
            <DashboardSearch title={'Projects'} fetchData={fetchProjectsSearch} handleRemoveSearch={handleRemoveSearch} handleChangeSearch={handleChangeSearch} query={query} handleSearch={handleSearch} filtersMenu={filtersMenu}/>
