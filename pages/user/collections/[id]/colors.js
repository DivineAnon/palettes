...
import { selectDetailDashboardCollection, selectDetailDashboardCollectionColors, setDetailDashboardCollection, setDetailDashboardCollectionColors } from "../../../../slices/dashboardSlice";
...
export default function CollectionColors(){
    ...
    const colors = useSelector(selectDetailDashboardCollectionColors);
    ...
    const fetchColorsSearch = async (query) => {
        ...
        const { data: { collection, colors: newColors } } = await axios.post(`${process.env.NEXT_PUBLIC_API}/api/collections/detail/${id}?${getQuery(query)}`,{
            view: 'colors',
            page: 1
        },{
            headers: {
        ...
        dispatch(setDetailDashboardCollectionColors(newColors));
    }
    const fetchColors = async () => {
        setLoadingFetch(true);
        const { data: { collection, palettes: newColors } } = await axios.post(`${process.env.NEXT_PUBLIC_API}/api/collections/detail/${id}?${getQuery(query)}`,{
            view: 'colors',
            page: parseInt(colors.meta.pagination.page)+1
        },{
            headers: {
                Authorization: `bearer ${GetToken()}`
            }
        ...
        dispatch(setDetailDashboardCollectionColors({ ...colors, data: colors.data.concat(newColors.data), meta: newColors.meta }));
    }
    return (
        <PCTemplate name={collection.name} collections={[collection]}>
            <PCSearchTemplate name={collection.name} fetchData={fetchColorsSearch} handleRemoveSearch={handleRemoveSearch} handleChangeSearch={handleChangeSearch} handleSearch={handleSearch} filtersMenu={filtersMenu} title={`Colors (${colors?.count})`} menuMore={menuMore} query={query}/>
...
export const getServerSideProps = wrapper.getServerSideProps(store => async (ctx) => {
    try {
        ...
        store.dispatch(setDetailDashboardCollectionColors(data.colors));
    } catch (error) {
