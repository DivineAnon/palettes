...
export default function CollectionGradients(){
    ...
    const fetchGradientsSearch = async (query) => {
        ...
        const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API}/api/collections/detail/${id}?${getQuery(query)}`,{
            view: 'gradients',
            page: 1
        },{
            headers: {
        ...
        dispatch(setDetailDashboardCollectionGradients(data.gradients));
    }
    const fetchGradients = async () => {
        setLoadingFetch(true);
        const { data: { collection, gradients: newGradients } } = await axios.post(`${process.env.NEXT_PUBLIC_API}/api/collections/detail/${id}?${getQuery(query)}`,{
            view: 'gradients',
            page: parseInt(gradients.meta.pagination.page)+1
        },{
            headers: {
                Authorization: `bearer ${GetToken()}`
            }
        ...
        dispatch(setDetailDashboardCollectionGradients({ ...gradients, data: gradients.data.concat(newGradients.data), meta: newGradients.meta }));
    }
    return (
        <PCTemplate name={collection.name} collections={[collection]}>
            <PCSearchTemplate name={collection.name} fetchData={fetchGradientsSearch} handleRemoveSearch={handleRemoveSearch} handleChangeSearch={handleChangeSearch} handleSearch={handleSearch} filtersMenu={filtersMenu} query={query} title={`Gradients (${gradients?.count})`} menuMore={menuMore}/>
