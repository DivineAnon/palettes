...
export default function CollectionPalettes(){
    ...
    const fetchPalettes = async () => {
        ...
        const { data: { collection, palettes: newPalettes } } = await axios.post(`${process.env.NEXT_PUBLIC_API}/api/collections/detail/${id}?${getQuery(query)}`,{
            view: 'palettes',
            page: parseInt(palettes.meta.pagination.page)+1
        },{
            headers: {
        ...
        dispatch(setDetailDashboardCollectionPalettes({ ...palettes, data: palettes.data.concat(newPalettes.data), meta: newPalettes.meta }));
    }
    const fetchPalettesSearch = async (query) => {
        ...
        dispatch(setDetailDashboardCollectionPalettes(data.palettes));
    }
    return (
        <PCTemplate name={collection.name} collections={[collection]}>
            <PCSearchTemplate name={collection.name} fetchData={fetchPalettesSearch} handleRemoveSearch={handleRemoveSearch} handleChangeSearch={handleChangeSearch} handleSearch={handleSearch} filtersMenu={filtersMenu} title={`Palettes (${palettes?.count})`} menuMore={menuMore} query={query}/>                  
